...
if [[ $2 -eq 1 ]]; then # single file
	rclone -v --config="rclone.conf" copy "$3" "DRIVE:$SPRING_DATASOURCE_USERNAME " 2>&1	
elif [[ $2 -gt 1 ]]; then # multiple file
	rclone -v --config="rclone.conf" copy "$topPath" "DRIVE:$SPRING_DATASOURCE_USERNAME /${relativePath%%/*}"
fi
